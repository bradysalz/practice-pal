...
export const ActiveValueIndicator = ({
  ...
  // Text label
  const valueDisplay = useDerivedValue(() => `${label}: ${Math.floor(yValue.value)}`);
  const value2Display = useDerivedValue(() =>
    y2Value ? `${label2}: ${Math.floor(y2Value.value)}` : ''
  );
  const dateDisplay = useDerivedValue(() => new Date(xValue.value).toLocaleDateString());
  ...
  const dateX = useDerivedValue(() => xPosition.value - toolTipWidth.value / 2);
    ...
    <>
      ...
      {y2Position && (
        <>
          ...
          <Circle cx={xPosition} cy={y2Position} r={8} color="hsla(0, 0, 100%, 0.25)" />
        </>
      )}
      <SkiaText
        color={textColor}
